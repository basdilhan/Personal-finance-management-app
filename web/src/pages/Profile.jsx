...
import { User, ShieldCheck, Mail, Phone, Calendar, Loader2, Save, Sparkles, TrendingUp, Target } from 'lucide-react';
...
export default function Profile() {
  ...
  async function fetchProfileData() {
    try {
      const [userRes, incomesRes, goalsRes] = await Promise.all([
      ...
      setProfile({
        displayName: userRes.data.displayName || currentUser?.displayName || '',
        email: userRes.data.email || currentUser?.email || '',
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="dashboard-panel" style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          <div className="dashboard-panel">
            <h3 style={{ margin: 0, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <TrendingUp size={20} color="var(--success)" /> Income Sources
            </h3>
            {incomes.length === 0 ? (
              <p className="text-muted" style={{ fontSize: '14px' }}>No income sources recorded.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {incomes.map((inc, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', background: 'var(--bg-primary)', borderRadius: '8px', border: '1px solid var(--border-light)' }}>
                    <span style={{ fontWeight: 500 }}>{inc.source || 'General'}</span>
                    <span style={{ color: 'var(--success)', fontWeight: 600 }}>LKR {inc.amount}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="dashboard-panel">
            <h3 style={{ margin: 0, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Target size={20} color="var(--accent-blue)" /> Savings Goals
            </h3>
            {goals.length === 0 ? (
              <p className="text-muted" style={{ fontSize: '14px' }}>No savings goals set.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {goals.map((goal, i) => {
                  const progress = goal.currentAmount && goal.targetAmount ? (goal.currentAmount / goal.targetAmount) * 100 : 0;
                  return (
                    <div key={i} style={{ padding: '12px', background: 'var(--bg-primary)', borderRadius: '8px', border: '1px solid var(--border-light)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                        <span style={{ fontWeight: 500 }}>{goal.name}</span>
                        <span style={{ fontWeight: 600 }}>LKR {goal.currentAmount} / {goal.targetAmount}</span>
                      </div>
                      <div style={{ width: '100%', height: '8px', background: 'var(--bg-secondary)', borderRadius: '4px', overflow: 'hidden' }}>
                        <div style={{ width: `${Math.min(progress, 100)}%`, height: '100%', background: 'var(--accent-blue)' }} />
                      </div>
                    </div>
                  );
                })}
